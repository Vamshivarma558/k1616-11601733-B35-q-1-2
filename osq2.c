...
{ 
  ...
  time=arrivaltime[0];
printf("Enter Time Quantum: for 1st iteration as per th question time slice for 1st iteration is 3) \t\n"); 
  scanf("%d",&timequantum); 
	...
	printf("Enter Time Quantum: for 2nd iterationas per question time slice for 2md is 6 \t\n"); 
  ...
  for(i=0;i<n;i++)
  {
	...
	printf("searching for minimum burst time\n");
	for(i=0;i<n;i++)
	{
		if(runningtime[i]>0)
		{
			minimum=runningtime[i];
		}
	}
	printf("burst time minimum is%d\n",minimum);
	if(minimum!=0){
		for(j=0;j<n;j++)
		{
			m=0;
			...
			for(i=0;i<n;i++)
			{
				if(runningtime[i]<=minimum && runningtime[i]!=0)
				{
					minimum = runningtime[i];
					l=i;
				}
			...
			m=l;		
			printf("the process P[%d]   having minimum burst time %d is running at present\n",m+1,runningtime[l]);
			time+=runningtime[l];
			runningtime[l]=0;
